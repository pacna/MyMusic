...
export default function ArtistsPage(): JSX.Element {
    ...
    const createViewModelForArtists = (response: MusicResponse[]): void => {
        const artistsVM: Artist[] = response?.reduce(
            (artists: Artist[], music: MusicResponse) => {
                const findArtistNameIndex: number = artists.findIndex(
                    (x: Artist) => x.name.toLowerCase() === music.artist
                );
                if (hasValidIndex(findArtistNameIndex)) {
                    const locatedArtist: Artist = artists[findArtistNameIndex];
                    const findAlbumNameIndex: number =
                        locatedArtist.albums.findIndex(
                            (x: Album) => x.title.toLowerCase() === music.album
                        );
                    if (hasValidIndex(findAlbumNameIndex)) {
                        locatedArtist.albums[findAlbumNameIndex].songs.push(
                            convertToAlbumSong(music.title, music.path)
                        );
                    } else {
                        locatedArtist?.albums.push(
                            convertToAlbum(music.album, music.title, music.path)
                        );
                    }
                } else {
                    ...
                    <Accordion key={index}>
                        <AccordionSummary expandIcon={<ExpandMore />}>
                            <Typography>
                                {artist.name +
                                    " " +
                                    displayNumberOfAlbums(artist.albums)}
                            </Typography>
                        </AccordionSummary>
